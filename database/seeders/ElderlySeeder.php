...
class ElderlySeeder extends Seeder
{
    public function run(): void
    {
        ...
        $scheme1 = PensionScheme::where('scheme_code', 'SCBP-001')->first();
        $scheme2 = PensionScheme::where('scheme_code', 'WPS-002')->first();
        $scheme3 = PensionScheme::where('scheme_code', 'DPS-003')->first();
        ...
        foreach ($citizens as $data) {
            ...
            if ($data['profile']['is_verified'] && $admin) {
                ...
                $profileData['verification_remarks'] = 'Profile verified successfully. All documents are valid.';
            }

            $profile = ElderlyProfile::firstOrCreate(['aadhaar_number' => $data['profile']['aadhaar_number']], $profileData);
            ...
            if ($data['scheme'] && $data['app_status']) {
                $application = PensionApplication::firstOrCreate(
                    ...
                    [
                        ...
                        'remarks'            => $data['app_status'] === 'rejected' ? 'Incomplete documentation provided.' : null,
                    ]
                );
                ...
                if ($data['app_status'] === 'approved') {
                    Notification::firstOrCreate(
                        ...
                        [
                            'message' => "Your pension application has been approved. You will receive ₹" . number_format($data['scheme']->monthly_amount, 2) . " per month.",
                            'type'    => 'success',
                            'is_read' => false,
                        ]
                    );
